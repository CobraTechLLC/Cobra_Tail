...
- **Physical attacks on hardware.** No side-channel resistance, no tamper-evident enclosures, no HSM. The Vault's on-disk ML-KEM private key is wrapped in AES-GCM, but the wrapping key is derived from device identifiers (device_id and machine_id) stored in plaintext on the same SD card. This is not meaningful protection against an attacker with physical possession of the Vault — anyone with the SD card can reconstruct the wrapping key in under a minute. The real defense against Vault theft is the 4-hour keypair regeneration cycle: every 4 hours the Vault destroys its old private key and generates a fresh one from new ESP32 entropy. A stolen Vault gives an attacker, at worst, the ability to decrypt up to ~4 hours of previously-captured tunnel traffic (and only if they also captured the corresponding ML-KEM ciphertext) and to impersonate the Vault for up to ~4 hours until the legitimate Vault's next automatic rotation invalidates the stolen key. The attacker cannot join the mesh as a new node from the Vault key alone — that still requires the Lighthouse TLS certificate fingerprint and the enrollment flow. If you notice the Vault is missing, shut down the Lighthouse or force an immediate rotation. Hardening the at-rest key with a passphrase or external USB unlock token is on the roadmap.
